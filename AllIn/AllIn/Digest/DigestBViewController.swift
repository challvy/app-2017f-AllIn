...
class DigestBViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        progressView = UIProgressView(frame: CGRect(x: 10, y: 168, width: mainTitleField.frame.size.width-20, height: 1))
        progressView.progress = 0.0
        progressView.trackTintColor = UIColor(displayP3Red:235/255, green:235/255, blue:235/255, alpha:1)
        progressView.tintColor = UIColor(displayP3Red:65/255, green:171/255, blue:225/255, alpha:1)
            ...
            if curSource == "Hupu" {
                ...
                let dataTask = session.dataTask(with: req){ (data, response, error) -> Void in
                    if error != nil{
                        ...
                        for (index, rssItem) in hupuRssParser.rssItems.enumerated() {
...
extension DigestBViewController: MenuViewControllerDelegate{
    ...
    func didClickBackgroundImgView() {
        self.delegate?.didClickBackgroundImg()
    }
